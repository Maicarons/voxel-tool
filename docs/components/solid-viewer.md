# Component Example · Solid VoxViewer

A complete, runnable SolidJS example: it builds a "gray base + rainbow sphere" model on the fly with `@voxel-tool/core`,
...
export function App() {
  ...
  const [fileName, setFileName] = createSignal('Built-in sample (gray base + rainbow sphere)');
  ...
  return (
    <div style={{ padding: '24px', display: 'flex', 'flex-direction': 'column', gap: '16px', 'align-items': 'flex-start' }}>
      <h1 style={{ margin: 0, 'font-size': '20px' }}>@voxel-tool/solid · Voxel Model Viewer</h1>
...
Source: [`packages/solid/example/`](https://github.com/Maicarons/voxel-tool/tree/main/packages/solid/example).
