...
SolidJS 3D 查看器组件（基于 `@voxel-tool/viewer` + Three.js）。真 3D 立方体 + 深度缓冲 + 面剔除 + 正交等距相机 + OrbitControls。
...
export function App() {
  const [data, setData] = createSignal<任何>(null);
...
详见 [组件示例 · Solid](/zh/components/solid-viewer)。
