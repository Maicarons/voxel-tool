...
Preact 3D viewer component for MagicaVoxel `.vox` voxel models, built on `@voxel-tool/viewer` + Three.js. It uses the same "true 3D" rendering approach as MagicaVoxel.
...
```bash
npm install @voxel-tool/preact @voxel-tool/core three
...
```js
import { VoxViewer, buildVoxelGeometry } from '@voxel-tool/preact';
```

- `VoxViewer`: Preact component wrapping the Three.js scene / camera / lights / `OrbitControls` / resource cleanup.
