...
Qwik 3D viewer component for MagicaVoxel `.vox` voxel models, built on `@voxel-tool/viewer` + Three.js. It uses the same "true 3D" rendering approach as MagicaVoxel.
...
```bash
npm install @voxel-tool/qwik @voxel-tool/core three
...
```js
import { VoxViewer, buildVoxelGeometry } from '@voxel-tool/qwik';
```

- `VoxViewer`: Qwik component wrapping the Three.js scene / camera / lights / `OrbitControls` / resource cleanup.
