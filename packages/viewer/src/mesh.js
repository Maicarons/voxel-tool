...
// (外表面积是 O(表面积) 而非 O(体素数))。与朴素算法在「暴露面 multiset」上完全
// 等价——只改变几何紧凑度，不改渲染外观。用于性能敏感路径。
// ===========================================================================
...
export function buildVoxelGeometryGreedy(voxels, palette) {
  ...
  for (const q of quads) appendGreedyQuad(arrays, q, min, palette, pushRawColor);
...
export function buildVoxelBucketsGreedy(voxels, palette, materials) {
  ...
  for (const q of quads) {
    ...
    appendGreedyQuad(getBucket(mid), q, min, palette, pushRawColor);
  }
  const out = [];
