// @voxel-tool/viewer 的纯逻辑验证 (无需浏览器):
// 验证 buildVoxelGeometry 的「面剔除」是否正确 —— 这是正经 voxel viewer 的性能核心。
// 通过相对路径直接引用 @voxel-tool/core 源码, 避免依赖其 dist 产物是否已构建。
...
console.log(ok ? 'VIEWER MESH OK' : 'VIEWER MESH FAILED');
process.exit(ok ? 0 : 1);
