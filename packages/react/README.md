...
MagicaVoxel `.vox` 体素模型的 **React 3D 查看器组件包**，底层基于 [`@vox/base`](../js)。采用与 MagicaVoxel 一致的「真 3D」渲染原理（参考 threejs-vox-loader / coding.kiwi 的 *Rendering .vox Files*）。
...
```bash
npm install @vox/react @vox/base three
# peerDependencies: react / react-dom (^18 或 ^19)
```
...
```jsx
import { VoxViewer } from '@vox/react';
...
| `width`      | `number`                          | 画布宽（px），默认 `480`                      |
| `height`     | `number`                          | 画布高（px），默认 `480`                      |
...
```bash
cd react
npm install        # 安装 react / vite / three 等 (会软链 ../js 作为 @vox/base)
...
```bash
npm run build      # 用 Vite 打成 dist (ESM), react/react-dom/three 作 externals
```

导出内容：

```js
import { VoxViewer, buildVoxelGeometry } from '@vox/react';
```

- `VoxViewer`：React 组件，封装 Three.js 场景 / 相机 / 光照 / `OrbitControls` / 资源清理。
